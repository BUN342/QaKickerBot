...
def gen_markup():
    ...
    markup.add(
        InlineKeyboardButton(
            "Создать игру", callback_data="create_game"), InlineKeyboardButton(
            "Вывести общую статистику", callback_data="allstat"), InlineKeyboardButton(
                "Вывести мою статистику", callback_data="mystat"), InlineKeyboardButton(
                    "Шутка дня", callback_data="top_joke"), InlineKeyboardButton(
                        "Анекдот дня", callback_data="top_anekdot"))
    return markup
...
def writeOnAGame(now_chat, message, from_who):
    ...
    if (isMe == 1):
        bot.answer_callback_query(
            message.id,
            '%s, в данный момент идёт игра, жди.' %
            from_who)
    elif (isMe == 2):
        bot.answer_callback_query(
            message.id,
            '%s, игроков уже достаточно.' %
            from_who)
    elif (isMe == 3):
        bot.answer_callback_query(
            message.id,
            '%s, ты уже в игре, дай другим записаться.' %
            from_who)
    elif (isMe == 4):
        bot.answer_callback_query(
            message.id,
            '%s, и куда ты регаться пытаешься?' %
            from_who)
    else:
        bot.send_message(
            message.message.chat.id, '%s, ты записался.\nСейчас %s/4 игроков в игре.' %
            (from_who, len(isMe)))
...
def stopGame(now_chat, message, from_who):
    ...
    if (isGameStop == 0):
        ...
        bot.answer_callback_query(message.id,
                                  'Игра даже не началась, отменять нечего.')
        return
    elif (isGameStop == 2):
...
def winGame(now_chat, message, from_who):
    ...
    if (isResult == 0):
        ...
        bot.send_message(
            message.message.chat.id,
            '%s объявил победу своей команды.' %
            from_who)
...
def loseGame(now_chat, message, from_who):
    ...
    if (isResult == 0):
        ...
        bot.send_message(
            message.message.chat.id,
            '%s объявил поражение своей команды.' %
            from_who)
